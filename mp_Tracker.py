...
class Tracker(SLAMParameters):
    def __init__(self, slam):
        ...
        self.cam_intrinsic = np.array([[self.fx, 0., self.cx],
                                       [0., self.fy, self.cy],
                                       [0., 0., 1]])
        self.output_paths = slam.output_path
    ...
    def tracking(self):
        ...
        self.rgb_images, self.depth_images = self.get_images(
            f"{self.dataset_path}/results")
        self.num_images = len(self.rgb_images)
        self.reg.set_max_correspondence_distance(
        ...
        for ii in range(self.num_images):
            ...
            # print('image shapes: ',current_image.shape, depth_image.shape)
            ...
            if self.iteration_images == 0:
                ...
                if self.rerun_viewer:
                    ...
                    rr.set_time_seconds(
                        "log_time", time.time() - self.total_start_time)
                    rr.log(
                        "cam/current",
                ...
                points = np.matmul(R, points.transpose()
                                   ).transpose() - np.matmul(R, T)
                ...
                input_filter[(trackable_filter)] = [
                    range(1, num_trackable_points+1)]
                ...
                if self.rerun_viewer:
                    ...
                    rr.set_time_seconds(
                        "log_time", time.time() - self.total_start_time)
                    rr.log(f"pt/trackable/{self.iteration_images}",
                           rr.Points3D(points, colors=colors, radii=0.02))
                np.save(
                    f"{self.output_paths}/pcd/pcd_target_{self.iteration_images}.npy", points)
            else:
                # print('source points:', points.shape)
                np.save(
                    f"{self.output_path}/pcd/pcd_source_{self.iteration_images}.npy", points)
                ...
                # get associated points source points
                target_corres, distances = self.reg.get_source_correspondence()

                # Keyframe selection #
                # Tracking keyframe
                len_corres = len(np.where(distances < self.overlapped_th)[
                                 0])  # 5e-4 self.overlapped_th
                ...
                if if_tracking_keyframe:
                    ...
                    not_overlapped_indices_of_trackable_points = self.eliminate_overlapped2(
                        distances, self.overlapped_th2)  # 5e-5 self.overlapped_th
                    ...
                    np.save(
                        f"{self.output_path}/pcd/pcd_target_{self.iteration_images}.npy", target_points)

                    self.reg.set_target_covariances_fromqs(
                        target_rots.flatten(), target_scales.flatten())
            ...
            # 30. float(self.test)
            while 1/((time.time() - self.total_start_time)/(self.iteration_images+1)) > 30.:
                time.sleep(1e-15)
        ...
        # Tracking end
        pbar.close()
        print(self.final_pose.shape, torch.tensor(self.poses).shape)
        self.final_pose[:, :, :] = torch.tensor(self.poses).float()
        self.end_of_dataset[0] = 1

        with open(f"{self.output_path}/pred_poses.txt", "w") as f:
        ...
        with open(f"{self.output_path}/gt_poses.txt", "w") as f:
            for i, pose in enumerate(self.gt_poses):
                data = [i, *pose[:3, 3], *
                        Rotation.from_matrix(pose[:3, :3]).as_quat()]
                f.write(' '.join([str(elem) for elem in data]) + '\n')

        print(
            f"System FPS: {1/((time.time()-self.total_start_time)/self.num_images):.2f}")
        print(
            f"ATE RMSE: {self.evaluate_ate(self.gt_poses, self.poses)*100.:.2f}")
    ...
    def get_images(self, images_folder):
        ...
        for i in tqdm(range(len(self.dataset))):
            gt_pose, rgb, depth = self.dataset[i]
            rgb_images.append(rgb)
            depth_images.append(depth)
            self.gt_poses.append(np.array(gt_pose))
        self.poses.append(self.gt_poses[0])
    ...
    def downsample_and_make_pointcloud2(self, depth_img, rgb_img):
        ...
        colors = torch.from_numpy(
            rgb_img).reshape(-1, 3).float()[self.downsample_idxs]/255
        z_values = torch.from_numpy(depth_img.astype(np.float32)).flatten()[
            self.downsample_idxs]/self.depth_scale
    ...
    def align(self, model, data):
        ...
        if (np.linalg.det(U) * np.linalg.det(Vh) < 0):
            S[2, 2] = -1
        rot = U*S*Vh
        trans = data.mean(1).reshape((3, -1)) - rot * \
            model.mean(1).reshape((3, -1))
    ...
    def evaluate_ate(self, gt_traj, est_traj):
        ...
        gt_traj_pts = torch.stack(
            tuple(gt_traj_pts_tensor)).detach().cpu().numpy().T

        est_traj_pts = [est_traj[idx][:3, 3] for idx in range(len(est_traj))]
        est_traj_pts_arr = np.array(est_traj_pts)
        est_traj_pts_tensor = torch.tensor(est_traj_pts_arr)
        est_traj_pts = torch.stack(
            tuple(est_traj_pts_tensor)).detach().cpu().numpy().T
